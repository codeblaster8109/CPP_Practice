...
int main(){
    fastio
    int friends,bottles,drink_amount_in_each_bottle,limes,slices,salt,drink_reqd,salt_reqd;
    cin>>friends>>bottles>>drink_amount_in_each_bottle>>limes>>slices>>salt>>drink_reqd>>salt_reqd;
    int total_drink=drink_amount_in_each_bottle*bottles;//9
    int total_lime_slice=slices*limes;//25
    int count=0;
    for(int i=0;total_drink>=drink_reqd&&total_lime_slice>=1&&salt>=salt_reqd;i++)
    {
        count++;
        total_drink-=drink_reqd;
        total_lime_slice--;
        salt-=salt_reqd;
    }
    if(friends!=0)
