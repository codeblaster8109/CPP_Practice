...
ll MaxPathSum(ll arr2D[][10],ll n,ll m,ll i,ll j)
{
    if(i>=n||j>=m)
        return INT_MIN;
    ll down=INT_MIN;
    ll right=INT_MIN;
    if(i<n)
        right=MaxPathSum(arr2D,n,m,i+1,j);
    if(j<m)
        down=MaxPathSum(arr2D,n,m,i,j+1);
    if(down==INT_MIN&&right==INT_MIN)
    {
        down=0;
        right=0;
    }
    return arr2D[i][j]+max(right,down);
}
